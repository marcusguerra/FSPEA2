 ...
 * </ol>
 */
public class ESPEA3 extends AbstractEvolutionaryAlgorithm implements EpsilonBoxEvolutionaryAlgorithm{
	 ...
	 */
	public ESPEA3(Problem problem, int initialPopulationSize, Initialization initialization,
			Variation variation,EpsilonBoxDominanceArchive archive, int numberOfOffspring, int k) {
	...
	protected void iterate() {
		...
		EpsilonBoxDominanceArchive offspring = getArchive();
		...
		if (offspring.size() <= 1) {
			...
			parents = ArrayUtils.add(
					...
					offspring.get(PRNG.nextInt(offspring.size())));
		}
		...
		population.addAll(truncate(offspring, offspring.size()));
	 ...
	 */
	protected Population truncate(EpsilonBoxDominanceArchive a, int size) {
