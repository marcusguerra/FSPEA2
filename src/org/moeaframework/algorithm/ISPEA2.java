 ...
 * 
 * ORIGINAL PAPER
 * AN IMPROVED SPEA2 MULTI OBJECTIVE
	ALGORITHM WITH NON DOMINATED
	ELITISM AND GENERATIONAL CROSSOVER
 */
...
import org.moeaframework.util.TypedProperties;
  ...
  * </ol>
  */
 public class ISPEA2 extends AbstractEvolutionaryAlgorithm {
      ...
      */
     public ISPEA2(Problem problem, int initialPopulationSize, Initialization initialization,
             Variation variation, int numberOfOffspring, int k, int q) {
     ...
     public ArrayList<Population> listPopulation = new ArrayList<>();
 
     public void addList(Population pop){
         this.listPopulation.add(pop);
     }
     public Population getPop(int index){
         return this.listPopulation.get(index);
     }
     ...
     protected void iterate() {
		...
		//esse parents é o mating pool
		// etapa 5(parents) e 6(children)
		...
		while (offspring.size() < numberOfOffspring) {
			...
			while (offspring.size() < numberOfOffspring) {
				//concatenando as 2 pools
				Solution[] parents = selection.select(variation.getArity(), getPop(q-1));
				Solution[] parents1 = selection.select(variation.getArity(), getPop(q-2));
				Solution[] parentsPool = ArrayUtils.addAll(parents, parents1);
			
				population2.addAll(parentsPool);
				population2.sort(fitnessComparator);
				

				while(population2.size()>offspring.size()+1){
					population2.remove(0);
				}
				
				Solution[] finalPool = selection.select(variation.getArity(), population2);
				Solution[] children = variation.evolve(finalPool);
